...
namespace SdetBootcampDay1.Exercises
{
    ...
    public class TakeHomeExercises
    {
         ...
         */
        [Test]
        public void order1FIFA()
        {
            var ordHand = new OrderHandler();
            ordHand.OrderAndPay(OrderItem.FIFA_24, 1);
            Assert.That(ordHand.GetStockFor(OrderItem.FIFA_24), Is.EqualTo(9));
         ...
         */
        [Test]
        public void order101FN()
        {
            var ordHand = new OrderHandler();
            var ae = Assert.Throws<ArgumentException>(() =>
            {
                ordHand.OrderAndPay(OrderItem.Fortnite, 101);
            });
         ...
         */
        [Test]
        public void orderNonitem()
        {
            var ordHand = new OrderHandler();
            var ae = Assert.Throws<ArgumentException>(() =>
            {
                ordHand.AddStock(OrderItem.DayOfTheTentacle, 100);
            });
